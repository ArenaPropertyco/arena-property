...
/**
 * HU-00 · RF-00.3 — «¿Qué es Arena Property?»: la frase, la explicación del
 * modelo, los tres pilares y la foto del apartamento Invictvs, con el botón a la
 * página de detalle (HU-41). El destino lo trae el manifiesto; el evento `cta`
 * lo registra la página en analítica (RF-00.8).
 */
...
<template>
  <UPageSection
    ...
    :description="t('home.model.tagline')"
  ...
  >
    <div class="grid gap-10 lg:grid-cols-5 lg:items-center">
      <div class="order-2 space-y-8 lg:order-1 lg:col-span-3">
        <div class="space-y-4">
          <p class="text-base leading-relaxed text-default">
            {{ t('home.model.description') }}
          </p>
          <p class="border-l-2 border-primary pl-4 font-display text-2xl leading-snug text-highlighted">
            {{ t('home.model.lead') }}
          </p>
        </div>
      ...
      <figure class="group order-1 overflow-hidden rounded-2xl lg:order-2 lg:col-span-2">
        <NuxtImg
