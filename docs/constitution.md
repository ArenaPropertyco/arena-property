...
10. **Interfaz por componentes.** Toda pantalla se compone de componentes de `app/components/` construidos sobre `@nuxt/ui`, con props y eventos tipados: reciben datos y emiten acciones, nada más. Las páginas solo orquestan (cargan datos con composables, pasan props, atienden eventos) y los layouts solo estructuran con los componentes de cabecera, navegación y panel de `@nuxt/ui`. Ningún formulario, tabla, alerta ni bloque de marcado repetible vive dentro de una página o un layout. *Verificable:* ninguna página ni layout contiene `<UForm`, `<UFormField`, `<UInput`, `<USelect`, `<UTable`, `<UBadge`, `<UAlert`, `<form`, `<input` ni `<table`; ningún componente consulta datos (`useSupabaseClient`, `useAsyncData`, `useFetch`, `$fetch`).
